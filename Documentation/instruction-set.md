...
|        ![︽][elv]`0xfffe` | `EXEC`       | `EXEC <c> [params]`            | Executes the OP code associated with the given number `c` and passes the given parameters `params` to the OP code |
|        ![︽][elv]`0xffff` | `KERNEL`     | `KERNEL <1/0>`                 | Enters (1) or exits (0) the privileged kernel execution mode |
